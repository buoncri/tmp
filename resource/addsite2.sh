...
# `-f` switch validation: check for missing switch, or `-f` param starts with dash, or no parameter was given
...
##echo `grep "^SECRET_KEY" "${SETTINGS_PY_FILE}"` > "${ROOT_DIR}/${DJANGO_PROJECT_NAME}/secret_key.py"
...
mkdir "${ROOT_DIR}/${DJANGO_PROJECT_NAME}/apps"
...
# tell Apache2 to listen on the specified port
